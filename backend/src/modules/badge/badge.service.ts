...
import type { GrantBadgeInput, TrustBadgeKind } from "@roomadda/shared";
...
export const badgeService = {
   ...
   * rejected. FEATURED is time-boxed by `durationDays`.
   */
  async grant(actor: Actor, listingId: string, input: GrantBadgeInput, ip?: string): Promise<TrustTag> {
    ...
    const now = new Date();
    const expiresAt = new Date(now.getTime() + input.durationDays * DAY_MS);
    const badge = await prisma.trustTag.upsert({
      where: { listingId_kind: { listingId, kind: "FEATURED" } },
      create: { listingId, kind: "FEATURED", source: "ADMIN", earnedAt: now, expiresAt },
      update: { source: "ADMIN", expiresAt, suspended: false, suspendedReason: null, suspendedById: null, suspendedAt: null },
    });
    await writeAudit({
      actorId: actor.id,
      ...
      targetId: listingId,
      ip,
      metadata: { durationDays: input.durationDays, expiresAt: expiresAt.toISOString() },
    });
    return badge;
  },
